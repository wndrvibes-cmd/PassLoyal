...
import type { CustomerVisit, LoyaltyProgram, RewardHistoryEntry } from "@/types/database";
...
export default function CustomerHistory({
  ...
  const timeline: TimelineEntry[] = [
    ...
    ...rewards.map((reward): TimelineEntry => ({ kind: "reward", date: reward.redeemed_at, data: reward })),
      ...
      {timeline.map((entry) => {
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[13.5px] text-slate-700">
                <span className="font-medium text-slate-900">Récompense « {reward.reward_name} »</span>{" "}
                · -{reward.points_used} pts
              </p>
              <p className="mt-0.5 text-[12px] text-slate-400">{formatDateTime(reward.redeemed_at)}</p>
            </div>
            <button
              type="button"
