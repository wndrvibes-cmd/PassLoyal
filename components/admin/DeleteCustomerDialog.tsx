...
import { adminDeleteCustomer } from "@/services/adminCustomers";
...
export default function DeleteCustomerDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await adminDeleteCustomer(supabase, customer.id);
      toast.success("Client supprimé.");
      onDeleted(customer.id);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Impossible de supprimer ce client.");
      setIsDeleting(false);
    }
  };
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <h2 id="delete-customer-title" className="mt-4 text-[15px] font-semibold tracking-tight text-slate-900">
          Supprimer {customer.first_name} {customer.last_name} ?
        </h2>
        <p className="mt-1.5 text-[13.5px] text-slate-500">
          Cette action est définitive et supprimera son historique de visites et de récompenses.
        </p>
